      ...
      >
        ...
        <div className="glass-card rounded-2xl p-6 space-y-4 mb-6">
          <label className="flex items-start gap-3 cursor-pointer group" onClick={() => setConsent1(!consent1)}>
            <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center flex-shrink-0 mt-0.5 transition-all ${consent1 ? 'bg-game-green border-game-green' : 'border-game-muted'}`}>
              {consent1 && <span className="text-sm">✓</span>}
            </div>
          ...
          <label className="flex items-start gap-3 cursor-pointer group" onClick={() => setConsent2(!consent2)}>
            <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center flex-shrink-0 mt-0.5 transition-all ${consent2 ? 'bg-game-green border-game-green' : 'border-game-muted'}`}>
              {consent2 && <span className="text-sm">✓</span>}
            </div>
        ...
        <button
          ...
          className={`w-full py-4 rounded-2xl font-display font-semibold text-lg transition-all ${canProceed ? 'gold-gradient text-game-bg game-shadow hover:scale-105 active:scale-95' : 'bg-game-card text-game-muted cursor-not-allowed'}`}
        >
          Continue →
        </button>
