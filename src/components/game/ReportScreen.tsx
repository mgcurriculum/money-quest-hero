...
const ReportScreen = () => {
  ...
  const handleShare = (platform: string) => {
    ...
    const urls: Record<string, string> = {
      ...
      linkedin: `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(window.location.origin)}&summary=${encoded}`,
    ...
    <div className="min-h-screen game-gradient px-4 py-6">
      <div className="max-w-md mx-auto">
        ...
        >
          ...
          <div className="relative w-36 h-36 mx-auto mb-3">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <motion.span
                className="text-4xl font-display font-bold gold-text"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: 1 }}
              >
          ...
          <div className="w-full h-56">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart data={radarData} cx="50%" cy="50%" outerRadius="70%">
                ...
                <PolarAngleAxis
                  dataKey="dimension"
                  tick={{ fill: 'hsl(var(--game-muted))', fontSize: 9 }}
                />
                <Radar
                  name="Score"
                  dataKey="score"
                  stroke="hsl(var(--game-gold))"
                  fill="hsl(var(--game-gold))"
                  fillOpacity={0.25}
                  strokeWidth={2}
                />
              </RadarChart>
              ...
              >
                ...
                <div className="h-2 bg-game-card rounded-full overflow-hidden">
                  <motion.div
                    ...
                    initial={{ width: 0 }}
                    animate={{ width: `${score}%` }}
                    transition={{ delay: 1.1 + idx * 0.08, duration: 0.6 }}
                  />
                </div>
              </motion.div>
            );
        ...
        <motion.div
          ...
          className="glass-card rounded-2xl p-4 mb-5 border-2 border-game-gold/30"
        >
          <p className="text-game-gold font-display font-semibold mb-2 text-sm">🎯 Your Improvement Quest</p>
          <p className="text-game-text font-body text-xs mb-1">
            <strong>Mission:</strong> {primaryArchetype.quest}
          </p>
